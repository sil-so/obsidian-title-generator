...
function useCompletionsEndpoint(model) {
  if (!model)
    return false;
  const lower = model.toLowerCase();
  return lower.includes("instruct") || lower.startsWith("text-") || lower.includes("davinci") || lower.includes("curie") || lower.includes("babbage") || lower.includes("ada");
}
function stripSurroundingQuotes(s) {
  return s.replace(/^[\s"'“”‘’]+|[\s"'“”‘’]+$/g, "");
}
function removeInvalidFileNameChars(s) {
  return s.replace(/[<>:"/\\|?*]/g, " ").replace(/\s+/g, " ").trim();
}
var TitleGeneratorSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.presetKey = "kebab";
    this.plugin = plugin;
  }
  display() {
    const { containerEl } = this;
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Provider").setDesc("Choose which API to use.").addDropdown((dd) => {
    ...
    if (this.plugin.settings.provider === "openai") {
      new import_obsidian.Setting(containerEl).setName("OpenAI API key").addText((text) => {
        ...
        text.setPlaceholder("API Key").setValue(this.plugin.settings.openAiApiKey).onChange(async (newValue) => {
          this.plugin.settings.openAiApiKey = newValue.trim();
          await this.plugin.saveSettings();
        });
      });
      new import_obsidian.Setting(containerEl).setName("OpenAI model").setDesc("Examples: gpt-4o-mini, gpt-4o, gpt-3.5-turbo, gpt-3.5-turbo-instruct").addText((text) => {
        text.inputEl.style.width = "100%";
        text.setPlaceholder("gpt-4o-mini").setValue(this.plugin.settings.openAiModel).onChange(async (newValue) => {
    ...
    if (this.plugin.settings.provider === "fireworks") {
      new import_obsidian.Setting(containerEl).setName("Fireworks API key").addText((text) => {
        text.inputEl.type = "password";
        text.inputEl.style.width = "100%";
        text.setPlaceholder("API Key").setValue(this.plugin.settings.fireworksApiKey).onChange(async (newValue) => {
          this.plugin.settings.fireworksApiKey = newValue.trim();
          await this.plugin.saveSettings();
        });
      });
      new import_obsidian.Setting(containerEl).setName("Fireworks model").setDesc(
        "Enter a Fireworks model ID (e.g., accounts/fireworks/models/llama-v3p1-8b-instruct)."
      ).addText((text) => {
        text.inputEl.style.width = "100%";
        text.setPlaceholder("accounts/fireworks/models/llama-v3p1-8b-instruct").setValue(this.plugin.settings.fireworksModel).onChange(async (newValue) => {
      ...
      note.textContent = "Fireworks calls use the Responses API with store=false by default for no retention.";
    }
    let promptAreaRef = null;
    new import_obsidian.Setting(containerEl).setName("Prompt presets").setDesc(
    ...
    const promptSetting = new import_obsidian.Setting(containerEl).setName("Custom prompt");
    ...
    tip.textContent = "Tip: End your prompt with \u201CReturn only the title.\u201D for the cleanest results.";
  }
};
var TitleGeneratorPlugin = class extends import_obsidian.Plugin {
  renderPrompt(content) {
    var _a3;
    const tmpl = ((_a3 = this.settings.customPrompt) == null ? void 0 : _a3.trim()) || DEFAULT_PROMPT;
    return tmpl.replace(/\{\{\s*content\s*\}\}/gi, content);
  }
  async callOpenAI(prompt) {
    var _a3, _b, _c, _d, _e, _f, _g, _h;
    const model = ((_a3 = this.settings.openAiModel) == null ? void 0 : _a3.trim()) || "gpt-5-nano-2025-08-07";
    if (!this.openai) {
    ...
    if (useCompletionsEndpoint(model)) {
      const response2 = await this.openai.completions.create({
        model,
        prompt,
        max_tokens: 50,
        temperature: 0.6
      });
      return ((_d = (_c = (_b = response2.choices) == null ? void 0 : _b[0]) == null ? void 0 : _c.text) != null ? _d : "").trim();
    }
    const response = await this.openai.chat.completions.create({
      ...
      verbosity: "low",
      // Concise output for titles
      n: 1
    });
    return ((_h = (_g = (_f = (_e = response.choices) == null ? void 0 : _e[0]) == null ? void 0 : _f.message) == null ? void 0 : _g.content) != null ? _h : "").trim();
  }
  async callFireworks(prompt) {
    var _a3, _b, _c, _d, _e, _f, _g, _h;
    const model = (_a3 = this.settings.fireworksModel) == null ? void 0 : _a3.trim();
    if (!model)
      throw new Error("Fireworks model is required.");
    const apiKey = (_b = this.settings.fireworksApiKey) == null ? void 0 : _b.trim();
    if (!apiKey)
    ...
    const body = {
      ...
      temperature: 0.6,
      store: false
      // explicitly disable storage by default
    };
    ...
    let out = ((_c = data.output_text) != null ? _c : "").toString().trim();
    if (!out && Array.isArray(data.output) && data.output.length > 0) {
      try {
        const last = data.output[data.output.length - 1];
        out = (_h = (_g = (_f = (_e = (_d = last == null ? void 0 : last.content) == null ? void 0 : _d[0]) == null ? void 0 : _e.text) == null ? void 0 : _f.toString()) == null ? void 0 : _g.trim()) != null ? _h : "";
      } catch (e) {
      }
    }
    return out;
  }
  async generateTitle(file, content) {
    ...
    try {
      ...
      console.error(err);
      new import_obsidian.Notice(`Unable to generate title:
...
${err}`);
  ...
  async generateTitleFromFile(file) {
    const content = await file.vault.cachedRead(file);
    return this.generateTitle(file, content);
  }
  async generateTitleFromEditor(editor) {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        ...
        menu.addItem((item) => {
          item.setTitle("Generate title").setIcon("lucide-edit-3").onClick(() => this.generateTitleFromFile(file));
        });
      })
    );
    this.registerEvent(
      this.app.workspace.on("files-menu", (menu, files) => {
        const tFiles = files.filter((f) => f instanceof import_obsidian.TFile);
        if (tFiles.length < 1) {
          return;
        }
        menu.addItem((item) => {
          item.setTitle("Generate titles").setIcon("lucide-edit-3").onClick(
